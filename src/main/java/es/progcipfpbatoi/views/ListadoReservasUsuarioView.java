...
public class ListadoReservasUsuarioView {
    ...
    private AsciiTable buildASCIITable()  {
        ...
        view.addRow("*", "*", "*", "*", "*");
        view.addRule();
        view.addRow(null, null, null, null, "Listado Viajes");
        view.addRule();
    ...
    private void generarFilasReservas (AsciiTable tabla){
        ...
        for (Reserva r : reservas) {
            tabla.addRow(r.getId(), r.getViaje().getId(), r.getViaje().getPropietario().getUsername(), null, r.getNumPlazasSolicitadas());
            tabla.addRule();
        }
    }
}
